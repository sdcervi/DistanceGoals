...
/*		if (sortBy == 'progress-desc') {
		...
		} else if (sortBy == 'calendar-desc') {
			const today = new Date ();
			let endDateA;
			let endDateB;
			if (challenges[a].period) {
				const startDateA = new Date (challenges[a].start);
				endDateA = new Date (challenges[a].start);
				endDateA.setDate(endDateA.getDate() + challenges[a].period);
			}
			if (challenges[b].period) {
				const startDateB = new Date (challenges[b].start);
				endDateB = new Date (challenges[b].start);
				endDateB.setDate(endDateB.getDate() + challenges[b].period);
			}
			console.log (a);
			console.log (endDateA + ' || ' + endDateB);
			if (!endDateA && endDateB) {
				return 1;
			} else if (endDateA && !endDateB) {
				return -1;
		...
		} else if (sortBy == 'calendar-asc') {
			const today = new Date ();
			const startDateA = new Date (challenges[a].start);
			let endDateA = new Date (challenges[a].start);
			endDateA.setDate(endDateA.getDate() + challenges[a].period);
			const startDateB = new Date (challenges[b].start);
			let endDateB = new Date (challenges[b].start);
			console.log (endDateA + ' || ' + endDateB);
			endDateB.setDate(endDateB.getDate() + challenges[b].period);
			if (endDateA > endDateB) { // If a's property comes after b's property
				return -1;
